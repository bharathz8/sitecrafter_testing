...
1. **Each step (node)** performs a specific task
2. **State flows** between nodes, accumulating results
3. **Conditional routing** allows different paths based on conditions
4. **Loops** enable retry/repair mechanisms
...
```mermaid
graph TD
    A[Intent Router] -->|Create| B[Blueprint Node]
    A -->|Modify| M[Modification Analyzer]
    A -->|Question| C[Chat Response Node]
    ...
    H -->|Errors Found| I[Repair Node]
    I --> H
    H -->|No Errors| J[Complete]
```
...
| **Structure** | Generates config files (package.json, vite.config) | `package.json`, `tsconfig.json` |
| **Core** | Creates utility files (utils, layouts, router) | `App.tsx`, `Layout.tsx` |
| **Component** | Generates reusable UI components | `ProductCard.tsx`, `Navbar.tsx` |
...
1. **AI-Powered**: Uses LLM to understand natural language
2. **Multi-Step Workflow**: Not just one API call, but a full pipeline
3. **Self-Repairing**: Validates and fixes its own code
4. **Live Preview**: WebContainer runs code instantly
5. **Production-Ready**: Generates complete, deployable websites
6. **Modular Architecture**: Easy to add new features
